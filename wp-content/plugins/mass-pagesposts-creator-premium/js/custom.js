jQuery(document).ready(function() {
    ...
    jQuery('body').on('keyup', '#page_filter_chosen input', function() {
        ...
        if (valueCount >= 3) {
           jQuery('#page_filter_chosen ul li.no-results').html('<img src="' + plugin_vars.plugin_url + 'images/ajax-loader.gif">');
            var data = {
                'action': 'page_finder_ajax',
                'value': value
            };
            jQuery.post(ajaxurl, data, function(response) {
                jQuery('#page-filter').append(response);
            ...
            if (remainCount > 0) {
                jQuery('#page_filter_chosen ul li.no-results').html('Please enter ' + remainCount + ' or more characters');
            }
        }
    });
});