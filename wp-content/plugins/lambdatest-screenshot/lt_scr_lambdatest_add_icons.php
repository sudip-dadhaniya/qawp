...
function lt_scr_lambdatest_add_icons_to_pages_posts()
{
	...
	<script>
			...
			if (lt_src_currnet_page === 'lt_scr_detail_page') {
				...
				if (lt_scr_pages && lt_scr_pages.length) {
					if (lt_scr_pages[index].lt_scr_is_disable === "0") {
						localStorage.setItem('lt_scr_current_url', lt_scr_pages[index].lt_scr_page_link + '&key=' + localStorage.getItem('lt_scr_draft_preview_secret'));
					} else {
		...
		document.addEventListener("DOMContentLoaded", function(event) {
			...
			<?php }
		if ($pagenow === "post.php" && isset($_GET['post']) && isset($_GET['action']) && sanitize_text_field(trim($_GET['action'])) === "edit") {
			?>
