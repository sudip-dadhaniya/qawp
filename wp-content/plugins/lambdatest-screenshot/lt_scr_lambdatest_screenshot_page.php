                                    ...
                                    if (isset($cur_user) && isset($cur_user->email) && is_email(sanitize_email(trim($cur_user->email))) && isset($cur_user->token) && sanitize_text_field(trim($cur_user->token))) {?>
                                        axios.post('<?php echo $lt_lums_url;?>/api/user/token/auth', {
                                            ...
                                            if(response.status === 200){
                                                ...
                                                localStorage.setItem('lt_scr_organization_id',authData.organization.id);
                                                localStorage.setItem('lt_user_name',authData.username);
                                                this.sendScreenshot(authData.organization.id,false);
                                            }
                                        }.bind(this))
                                        .catch(function (error) {
                                ...
                                if (isset($cur_user) && isset($cur_user->email) && is_email(sanitize_email(trim($cur_user->email))) && isset($cur_user->token) && sanitize_text_field(trim($cur_user->token))) {?>
                                    axios.post('<?php echo $lt_lums_url;?>/api/user/token/auth', {
                                        ...
                                        if(response.status === 200){
                                            ...
                                            localStorage.setItem('lt_scr_organization_id',authData.user.organization.id);
                                            localStorage.setItem('lt_user_name',authData.username);
                                            this.sendScreenshot(authData.user.organization.id,false);
                                        }
                                    }.bind(this))
                                    .catch(function (error) {
                                        this.main_loader_is_active = false;
                                ...
                                if(this.isUrlValid(this.myDefaultConf.url)){
                                    ...
                                    axios.post('<?php echo $lt_falcon_url;?>/tests', this.myDefaultConf,{
                                        headers: {
                                            ...
                                            "accessToken": `${localStorage.getItem('lt_scr_access_token')}`,
                                            "username": `${localStorage.getItem('lt_user_name')}`
                                        }
                                    }).then(function (response) {
