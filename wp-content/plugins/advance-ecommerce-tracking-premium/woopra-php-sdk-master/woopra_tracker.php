 ...
 */
class AdvanceWoopraTracker {
 
	public static $SDK_ID = "php";
	/**
	...
	*/
	public static $default_config = array(
		"domain" => "", 
		"cookie_name" => "wooTrackerAdvanceTracking",
		"cookie_domain" => "",
		"cookie_path" => "/",
		"ping" => true,
		"ping_interval" => 12000,
	...
	*/
	...
	function __construct($config_params = null) {
		...
		$this->current_config = AdvanceWoopraTracker::$default_config;
		...
		//configure app ID
		$this->current_config["app"] = AdvanceWoopraTracker::$SDK_ID;
		$this->custom_config = array("app" => AdvanceWoopraTracker::$SDK_ID);
		...
		$this->current_config["cookie_value"] = isset($_COOKIE[$this->current_config["cookie_name"]]) ? $_COOKIE[$this->current_config["cookie_name"]] : AdvanceWoopraTracker::RandomString();
	...
	*/
	public function config($args) {
		...
		foreach( $args as $option => $value) {
			...
			if ( array_key_exists($option, AdvanceWoopraTracker::$default_config) ) {

				if ( gettype($value) == gettype( AdvanceWoopraTracker::$default_config[$option] ) ) {
					...
					trigger_error("Wrong value type in configuration array for parameter ".$option.". Recieved ".gettype($value).", expected ".gettype( AdvanceWoopraTracker::$default_config[$option] ).".");
				}
			}
			else {
