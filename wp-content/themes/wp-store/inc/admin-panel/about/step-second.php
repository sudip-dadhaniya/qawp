...
$wp_store_recommended_plugins = array(
	...
	<div class="feature-section recommended-plugins">
		<?php foreach ( $wp_store_recommended_plugins as $plugin => $prop ) { ?>
		<?php
		...
		switch ( $active['needs'] ) {
			...
			$label = __( 'Install', 'wp-store' );
			break;
			case 'activate':
			$class = 'activate-now button button-primary';
			$label = __( 'Activate', 'wp-store' );
			break;
			case 'deactivate':
			$class = 'deactivate-now button';
			$label = __( 'Deactivate', 'wp-store' );
			break;
		}
		...
		<div class="col plugin_box">
			<?php if ( $prop['recommended'] ): ?>
				<span class="recommended"><?php _e( 'Required', 'wp-store' ); ?></span>
			<?php endif; ?>
			...
			<div class="b-wrap">
			<span class="version"><?php echo __( 'Version:', 'wp-store' ); ?><?php echo esc_html( $info->version ) ?></span>
